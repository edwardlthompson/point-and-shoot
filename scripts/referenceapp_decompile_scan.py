...
# AltReferenceApp / RAW video / DCG (Milestone 13.1)
NEEDLES_ALTREFERENCEAPP = [
    "EnableHDRDCGMode",
...
PROFILES = {
    ...
    "altreferenceapp": NEEDLES_BASE + [n for n in NEEDLES_ALTREFERENCEAPP if n not in NEEDLES_BASE],
    "all": list(dict.fromkeys(NEEDLES_BASE + NEEDLES_ALTREFERENCEAPP)),
}

CLASS_HINT = re.compile(
    r"(?i)(camera|raw|dng|capture|sensor|lens|hal|photo|still|referencecam|altreferenceapp|mcraw)"
)
...
def parse_args(argv: list[str]) -> tuple[Path, Path, list[str]]:
    ...
    if len(args) < 1:
        print("usage: referenceapp_decompile_scan.py <jadx_sources_dir> [out.json] [--profile referencecam|altreferenceapp|all]")
        sys.exit(2)
    root = Path(args[0])
