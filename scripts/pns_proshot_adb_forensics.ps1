...
$outDir = Join-Path $projRoot "hfr-runs\proshot_adb_forensics_$ts"
...
Write-Host "[proshot_forensics] -> $outDir"
Write-Host "[proshot_forensics] dumping package activities / intent filters..."
$dump = Invoke-AdbOut @("shell", "dumpsys", "package", $ProShotPackage)
...
$logPath = Join-Path $outDir "proshot_capture_logcat.txt"
Invoke-Adb @(
...
Write-Host "[proshot_forensics] findings -> $(Join-Path $outDir 'proshot_adb_findings.md')"
Write-Host "[proshot_forensics] done; apps force-stopped"
