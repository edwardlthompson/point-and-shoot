...
object PnsLog {
    ...
    fun init(context: Context) {
        ...
        diagnosticsEnabled = runCatching { DiagnosticsMode.isEnabled(appCtx) }.getOrDefault(false)
        initialized = true
    }

    /** Refresh the cached diagnostics flag without re-reading `applicationInfo`. */
    fun refreshDiagnosticsFlag(context: Context) {
        diagnosticsEnabled = runCatching { DiagnosticsMode.isEnabled(context.applicationContext) }
            .getOrDefault(false)
    }

    fun v(tag: String, message: String) {
        if (verboseEnabled()) Log.v(prefixed(tag), message)
    }

    fun d(tag: String, message: String) {
        if (verboseEnabled()) Log.d(prefixed(tag), message)
    }
