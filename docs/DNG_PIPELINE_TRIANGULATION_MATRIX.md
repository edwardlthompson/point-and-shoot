# DNG pipeline triangulation — ReferenceCam vs AltReferenceApp vs Point & Shoot
...
**Reference runs:** `hfr-runs/referenceapp_reference_*`, `hfr-runs/aux_dng_capture_analyze_*`, `docs/DNG_PS_ALIGNMENT_SPIKE.md`, `docs/ALTREFERENCEAPP_APK_FLEET_ANALYSIS.md`.

---

## Pipeline stages (matrix)

Legend: **S** = same across all three (or same on legacy device leaf ids) · **D** = differs · **P** = P&S-only · **?** = unverified on device for AltReferenceApp DNG

| Stage | ReferenceCam (good color) | AltReferenceApp (good UX) | P&S ReferenceCam attempt | P&S AltReferenceApp-inspired (current legacy device) | Triage |
|-------|----------------------|-------------------------|---------------------|----------------------------------------|--------|
| **Rear leaf `cameraId`** | **3** UW, **2** wide, **4** tele | Same ids via native catalog | **S** | **S** | Ruled out — dumpsys + forensics |
| **API stack** | Java Camera2 | NDK `ACamera*` + native encode | Java Camera2 | Java Camera2 | AltReferenceApp **D**; P&S matches ReferenceCam |
| **DNG writer** | `DngCreator(chars, result)` | Native `RawEncoder` + Adobe DNG SDK | **S** (framework) | **S** (framework) | AltReferenceApp **D**; both refs ≠ native |
...
| **Still: aberration / distortion** | When advertised | Native post-process | ON on leaf | **OFF** (AltReferenceApp backend) | **D** — optional bisect ON |
...
| **Post-`writeImage` ASN patch** | None | N/A | **OFF** (`useLegacyLeafAuxColorReconcile=false`) | **OFF** (AltReferenceApp path) | **S** with ReferenceCam — May 2026 |
| **Post-`writeImage` CM/FM TIFF** | None | N/A | **OFF** (`useReferenceAppReferenceCalibration=false`) | **S** | Ruled out; see **`docs/DNG_OPENABILITY_REGRESSIONS.md`** |
| **Still request (leaf)** | ReferenceCam still IQ + HAL AE | Native | **`ReferenceAppLeafStillCaptureRequest`** (no readout manual latch) | **D** vs old P&S stack | May 2026 USB |
...
These hold for **ReferenceCam**, **AltReferenceApp**, and **P&S** on legacy SKU leaf stills:
...
4. **RAW_SENSOR delivery** on current P&S AltReferenceApp path (`rawFmt=32`) — aligns with AltReferenceApp’s “full sensor” RAW, not ReferenceCam’s “first matching format in 32,37,38,36”.
...
AltReferenceApp **does not** help if the issue is (A): native encoder still needs correct matrices from the same HAL unless it applies its own per-device profile JSON.
...
| **2** | **RAW format pick** ReferenceCam order vs `RAW_SENSOR@activeArray` | A/B: `stillDngBackend=FRAMEWORK_REFERENCEAPP` **only** raw pick, keep reconcile **off** |
| **3** | **Still `SHADING_MODE`** (tele map-only) | A/B: ReferenceCam full shading on tele still (watch capture reason=0) |
| **4** | **ReferenceCam optical correction** off on AltReferenceApp path | A/B: enable `applyReferenceAppOpticalCorrection` on leaf |
| **5** | **`LeafDngHalReconcile`** (ReferenceCam path only) | Already off on AltReferenceApp path; if (2) retried, keep off |
...
|------|--------|----------|------------|---------|
| baseline | AltReferenceApp-inspired (shipped) | 0.42 | 0.42 | no |
| E1 | skip `StillCaptureMetadata` | 0.40 | 0.40 | no |
| E1_50708 | E1 + skip 50708 | 0.40 | 0.43 | no |
| **E2** | **`FRAMEWORK_REFERENCEAPP` still IQ** | **0.33** | **0.03** | **yes** |
| E2_reconcile | E2 + force ASN reconcile | 0.31 | 0.04 | yes |
| E2_skipmeta | E2 + skip metadata | 0.38 | 0.04 | yes |
| E7_reconcile_mc | AltReferenceApp + ASN reconcile | 0.37 | 0.05 | no |

**Verified conclusions (do not guess):**

1. **`StillCaptureMetadata.applyToDngUri` is not the green/black cause** — E1 ≈ baseline.
2. **AltReferenceApp-inspired still IQ was the tele regression** — tele RGB mean **B≈9.9** (baseline) vs **B≈16.4** (E2); map-only tele shading + no ReferenceCam optical correction.
3. **Ship `FRAMEWORK_REFERENCEAPP` on legacy device** — ReferenceCam leaf RAW order + full still shading + aberration/distortion when advertised.
...
| ReferenceCam order + shading + ASN reconcile | `FRAMEWORK_REFERENCEAPP` | 32→37→38→36 | ASN TIFF | Broken until Exif fix | Still bad |
| Loadability fix | ReferenceCam | same | No Exif; ASN only | **PASS** | Still bad |
| **AltReferenceApp-inspired (current)** | `ALTREFERENCEAPP_INSPIRED` | RAW_SENSOR @ active | None | **PASS** | **Still bad** (user May 2026) |
