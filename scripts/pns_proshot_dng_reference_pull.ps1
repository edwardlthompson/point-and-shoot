...
$outDir = Join-Path $projRoot "hfr-runs\proshot_reference_$ts"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
Write-Host "[proshot_ref] -> $outDir"
...
if ($paths.Count -eq 0) {
    Write-Host "[proshot_ref] no non-P&S DNGs under DCIM; trying /sdcard"
...
if ($picked.Count -lt $DngCount) {
    Write-Warning "[proshot_ref] only found $($picked.Count) candidate DNG(s); expected $DngCount"
}
...
foreach ($f in $picked) {
    $i++
    $local = Join-Path $outDir ("proshot_{0:D2}.dng" -f $i)
    Write-Host "[proshot_ref] pull $($f.Path)"
    if ($Serial) {
...
foreach ($line in $pathLines) {
    if ($line -match "^package:(.+)") {
        ...
        Write-Host "[proshot_ref] pull APK $remoteApk"
...
if ((Test-Path $apkDir) -and (Test-Path $pyApkGrep)) {
    ...
    if ($baseApk) {
        ...
        Write-Host "[proshot_ref] apk string hits -> $apkStrings"
    }
}
