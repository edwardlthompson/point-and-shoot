...
Compares M14/M23/M73 captures to tests/fixtures/referenceapp_legacy_sku/ (or -ReferenceAppDir).
...
SLOTS = (
    ("uw", "referenceapp_uw_cam3.dng", "M14_uw.dng", "3"),
    ("wide", "referenceapp_wide_cam2.dng", "M23_wide.dng", "2"),
    ("tele", "referenceapp_tele_cam4.dng", "M73_tele.dng", "4"),
)
...
def main() -> int:
    ...
    for slot, ref_name, pns_name, _cam in SLOTS:
        ...
        if pro_asn and pns_asn:
            ...
            row["referenceapp_asn_wb"] = [round(pro_asn[0], 3), round(pro_asn[1], 3)]
        ...
        try:
            ...
            row["referenceapp_bayer"] = bayer_channel_means(ref_p)
            row["pns_bayer"] = bayer_channel_means(pns_p)
            g_mean = max(
                sum(row["referenceapp_bayer"]) / 4.0,
                sum(row["pns_bayer"]) / 4.0,
                1.0,
            )
            row["bayer_mean_delta"] = round(
                sum(abs(a - b) for a, b in zip(row["pns_bayer"], row["referenceapp_bayer"]))
                / g_mean,
                4,
            )
    ...
    for slot, row in results["slots"].items():
        ...
        if "asn_ok" in row:
            ...
            if "pns_asn_wb" in row:
                asn_s += f" ASN_WB pns={row['pns_asn_wb']} pro={row['referenceapp_asn_wb']}"
        bayer_s = ""
        if "bayer_ok" in row:
