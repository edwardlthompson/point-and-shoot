#!/usr/bin/env python3
"""Parse proshot_live_forensics artifacts: EXIF + Bayer means per lens."""
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: proshot_live_parse.py <forensics_out_dir>")
        return 1
    root = Path(sys.argv[1])
    rows = []
    for p in sorted(root.glob("proshot_*.dng")):
        info = analyze_dng(p)
    ...
    for r in rows:
        ...
        md.append("")
    (root / "proshot_live_parse.md").write_text("\n".join(md), encoding="utf-8")
    return 0
