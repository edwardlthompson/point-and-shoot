...
if (-not $SkipForensics) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "pns_proshot_live_forensics failed" }
    $latest = Get-ChildItem (Join-Path $projRoot "hfr-runs") -Directory -Filter "proshot_live_forensics_*" |
        Sort-Object Name | Select-Object -Last 1
    if (-not $latest) { throw "No proshot_live_forensics_* output" }
    $ForensicsDir = $latest.FullName
} elseif (-not [string]::IsNullOrWhiteSpace($ForensicsDir)) {
    if ($ForensicsDir -match "\*") {
        $ForensicsDir = (Get-ChildItem (Join-Path $projRoot "hfr-runs") -Directory -Filter "proshot_live_forensics_*" |
...
foreach ($name in @("proshot_uw_3.dng", "proshot_wide_2.dng", "proshot_tele_4.dng")) {
...
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "pns_proshot_reference_sync.ps1") `
...
if (-not $SkipPnsCapture) {
    ...
    $parityArgs = @("-File", (Join-Path $PSScriptRoot "pns_proshot_parity_gate.ps1"))
...
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "pns_proshot_parity_gate.ps1") `
