...
object BackCameraRoleResolver {
    ...
    fun resolve(cm: CameraManager, ids: List<String>): Roles =
        FleetRoleMergeHook.mergeRoles(resolveEnumerated(cm, ids), ids)
     ...
     */
    fun rolesFromEnumeratedPhysicalsForTests(idsAndFocals: List<Pair<String, Float?>>): Roles =
 ...
 */
fun telePhysicalForPreviewPin(slot: FocalMmSlot, roles: BackCameraRoleResolver.Roles): String? =
    when (slot) {
...
fun ultraWidePhysicalForPreviewPin(slot: FocalMmSlot, roles: BackCameraRoleResolver.Roles): String? =
...
fun resolveFocalMmSlot(context: Context, slot: FocalMmSlot, ids: List<String>): Pair<String, FocalMode?>? {
    ...
    val roles = FleetRoleMergeHook.mergeRoles(BackCameraRoleResolver.resolveEnumerated(cm, ids), ids)
 ...
 */
fun pickCameraIdFromM23Resolve(
