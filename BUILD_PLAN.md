...
- [x] **Color-science foundation modules shipped (Round 15)**: `HdrCurves.kt` ships sRGB / Rec.709 / PQ / HLG transfer functions (CIE / IEC / ITU-R / SMPTE standards data, no trademarked source) — every curve is a pure `Float -> Float` so the same primitives can be lifted across hot per-pixel loops, GLES shader generation (the GLSL form is one-to-one with the Kotlin form), and JVM unit tests; **JUnit-tested** in `HdrCurvesTest` (19 tests: round-trip parity for every direction, mid-gray / break-point / unity / zero anchors, monotonicity sweep). `ColorSpaceMatrix.kt` ships sRGB / Rec.709 / Rec.2020 / DCI-P3 / ACES AP1 primaries + Bradford D65↔D50 chromatic adaptation, with the Lindbloom-derived `linear-RGB → CIE XYZ` matrices computed on the fly from the chromaticities (so the constants table can be re-verified against any future spec errata) and the standard CIE 1931 illuminants (D50 / D55 / D65 / D75 / A / F2). Above-spectral-locus primaries (notably ACES AP1 red at `(0.713, 0.293)`) are explicitly accepted; only `y > 0` is enforced (chromaticity divides by `y`). **JUnit-tested** in `ColorSpaceMatrixTest` (18 tests: every published matrix matched against the canonical Lindbloom / BT.2020 / RP 431-2 / ICC reference, Bradford D65↔D50 round-trip identity, whitepoint-of-every-working-space lands on its declared illuminant, invert times original equals identity, singular-matrix rejection, chromaticity validation). These two modules are the foundation for downstream HDR-aware encoding (10-bit AVIF Rec.2020 + 12-bit JXL Rec.2020), color-managed LUT application, and the DNG `ColorMatrix1` / `ForwardMatrix1` derivation flow.
- [x] **Working-space presets + CICP codes shipped (Round 16)**: `WorkingSpace.kt` ties together `HdrCurves` (transfer functions) and `ColorSpaceMatrix` (primaries + chromatic adaptation) into a single named-preset API the capture engine can pass around as a single value. Six shipped presets: `WorkingSpace.SRGB` (default sRGB / BT.709 SDR display), `WorkingSpace.REC709_SDR` (broadcast SDR with the BT.709 OETF + narrow range), `WorkingSpace.REC2020_PQ` (HDR10 / 10-bit AVIF target with PQ EOTF), `WorkingSpace.REC2020_HLG` (broadcast HDR with HLG OETF), `WorkingSpace.DCI_P3` (Display P3 wide-gamut SDR with sRGB-style EOTF), `WorkingSpace.ACES_AP1_LINEAR` (linear-light intermediate working space; never bitstream-tagged directly). Every preset carries an ITU-T H.273 CICP triple (`colour_primaries` / `transfer_characteristics` / `matrix_coefficients` + `video_full_range_flag`) so downstream containers (AVIF, AV1, HEVC, VVC, MP4, WebP, JXL) can tag the bitstream so any decoder applies the inverse EOTF and gamut conversion correctly. The CICP codes match the ISO/IEC 14496-10 (H.264) / 23008-2 (H.265) / 23090-3 (VVC) tables — sRGB is `(1, 13, 0, full)`; Rec.2020 PQ is `(9, 16, 9, full)`; Rec.2020 HLG is `(9, 18, 9, full)`; Display P3 is `(12, 13, 0, full)`; Rec.709 SDR is `(1, 1, 1, narrow)`; ACES AP1 linear is `(2, 8, 0, full)` (the H.273 sentinel "unspecified" entries because no CICP code exists for AP1 + linear). Convenience methods `linearToXyz(rgb)` and `toXyzD65(linearRgb)` (Bradford-adapt source-whitepoint XYZ to D65 when needed) are exposed for cross-color-space comparisons. The companion's `ALL` list is `by lazy` (a non-lazy `val` would fail with `NullPointerException` at static init because of the sealed-class + nested-objects + companion-list circular dependency). `Cicp.describe()` produces a human-readable diagnostic string (e.g. `"BT.709 / sRGB / RGB-identity / full-range"`) for the diagnostics dump and probe results. **JUnit-tested** in `WorkingSpaceTest` (27 tests: every preset is in `ALL` in stable order, every preset has a unique id and non-blank displayName, `byId` resolves every shipped preset and returns null for unknown ids, every preset's CICP triple matches its declared spec, every preset's primaries match the corresponding `ColorSpaceMatrix.*_PRIMARIES`, transfer-function round-trip parity for every preset on a 33-point sweep, `linearToXyz` on white returns the working-space whitepoint XYZ, `toXyzD65` on a D65 working space matches `linearToXyz` exactly, `toXyzD65` on ACES AP1 differs noticeably from `linearToXyz` (because of the Bradford D60 → D65 adaptation), `Cicp` constructor rejects out-of-range fields, `describe()` formats sRGB / Rec.2020 PQ / Rec.709 SDR correctly, falls back to literal codes for unknown values).
...
- [x] **`.spi3d`** (Sony Pictures Imageworks; publicly documented) — secondary import-only. **Shipped (Round 13)** as `LutPipeline.parseSpi3d(text)`: parses the SPI 3D LUT format (`SPILUT 1.0` header, `<inputDim> <outputDim>` line — only `3 3` accepted; 1D variants are explicitly routed away with a `1D variants belong in a separate .spi1d parser` message — then a `<sizeR> <sizeG> <sizeB>` line — only cubic is accepted, only sizes in `Lut3D.SUPPORTED_SIZES` — and finally `size³` body rows of the form `<r_idx> <g_idx> <b_idx> <out_R> <out_G> <out_B>` in arbitrary order); body order is recovered by index, so cells can appear in any permutation; duplicate cells and out-of-range indices are explicit failures. `LutImportValidator.sniffFormat(text)` extends to `.spi3d` (`SPILUT` header is sniffed before `.cube`/`.3dl` keywords); `categorize(...)` maps the SPI-specific failure messages to the existing `FailureCategory` enum so the SAF toast UX stays uniform. **Unblocks** the AMPAS ACES rows + the Blender Filmic `desat65cube.spi3d` row above: the user can now drop a `.spi3d` file straight into the import flow and the validator routes it correctly. Tested in `Spi3dParserTest` (16 tests). is preserved. New `LutImportValidator.Format.{Cube, Dl3}` enum + `validateForFormat(text, format)` for round-trip tests. **JUnit-tested** in `Dl3ParserTest` (12 tests: identity 17-cube at 12-bit, identity 33-cube at 16-bit, leading comment + Mesh header tolerated, unsupported shaper size rejected, non-uniform shaper rejected, empty input rejected, malformed body row rejected, validator routes `.3dl` to parseDl3, sniffer picks Cube on `LUT_3D_SIZE`, sniffer picks Dl3 on integer-only first line, validator surfaces UnsupportedSize category, validator surfaces MalformedHeader category for empty `.3dl`).
...
- [ ] **RAW (DNG)**: never baked in. The active LUT name + SHA256 are written into the DNG's `UniqueCameraModel` / `Software` tag chain so desktop processors (`darktable`, `RawTherapee`) can apply the same LUT optionally. **Pure-data formatter shipped (Round 13)** as `DngLutMetadata.kt`: `formatSoftwareTag(appVersion, activeLut?)` produces the `Software` (TIFF tag 305) string `"Point & Shoot v<v>"` (no LUT) or `"Point & Shoot v<v> / LUT=<name> SHA256=<64-hex>"` (with a non-identity LUT) so desktop tooling can extract the LUT identity with a single regex (`MARKER_REGEX = LUT=([^\s]+)\s+SHA256=([a-f0-9]{64})`); `formatUniqueCameraModel(deviceModel, cameraId, ...)` defaults to `"<deviceModel> (cameraId=<id>)"` so per-device profiles in desktop RAW processors still resolve, with an opt-in `includeLutMarkerInUniqueCameraModel = true` for forensic capture where the LUT identity must round-trip even when `Software` gets overwritten. Sealed `LutIdentity` adapter (`Bundled(catalogId, sha256)` / `CubeFile(safeFilename, sha256)`) decouples the formatter from `LutSidecar` so the engine can also pass synthetic test patterns; init-time validation rejects whitespace in marker names and 64-hex SHA shape so the produced strings are guaranteed to round-trip through `parseLutMarker`. Tested in `DngLutMetadataTest` (20 tests). **Pending**: wire-up in `Dng12Saver.writeStill` once the Phase 1 capture engine lands.
...
### Root-only enhancements (opt-in; every feature has a non-root fallback)

**Policy.** Point & Shoot is designed so that **100 % of the user-visible features work on a stock, un-rooted device.** Root access is offered ONLY as a way to unlock peak performance / quality / diagnostics on devices that already have it (LineageOS user-debug, Magisk, KernelSU). Every root-only feature MUST advertise:
1. A clear *purpose* (what does it do beyond the non-root path?).
2. A clear *fallback* (what happens when root is not present? which non-root code path takes over?).
3. A non-deceptive label in the Root Only settings drawer (greyed out until the user grants SU; explains the consequence of granting).

**Settings layout.** The Root Only items live in their own settings drawer (`RootSettingsScreen`, reachable from `Settings > Root Only` and `--es pns_screen rootsettings`). The drawer always renders, even on stock devices, so the user can see what they would gain by rooting; rows are visibly disabled with the canonical reason ("Requires root. Fallback: <X>"). A single "Grant Su" button at the top performs an explicit `Process.exec("su -c id")` only when the user taps it — Point & Shoot never silently shells out to `su` on launch (silent SU prompts are the #1 complaint of root-aware apps).

**Detection.** Detection is **two-stage** by design:
* **Stage 1 (Static, runs on every cold start).** `RootCapabilityProbe.probeStatic()` walks a fixed list of canonical SU paths (`/system/xbin/su`, `/system/bin/su`, `/sbin/su`, `/system/app/Superuser.apk`, `/data/adb/magisk`, `/data/adb/ksu`) using `java.io.File.exists()` only. **No process is forked.** The result is `RootState.Unknown` (probe not yet run), `RootState.NotAvailable` (no SU binary or manager package found), or `RootState.AvailableNotGranted` (binary present, grant status unknown). This stage never triggers an SU prompt and is safe to run from `Application.onCreate`.
* **Stage 2 (Active, runs ONLY when the user taps "Grant Su").** `RootCapabilityProbe.requestGrant()` forks `Process.exec("su -c id")`, reads stdout for `uid=0`, and parks the result in `RootState.Granted` or `RootState.Denied`. This call BLOCKS off the main thread; the UI shows a spinner. A user-visible toast surfaces "Granted" / "Denied" / "No SU manager".

**Root-only enhancements (every entry must enumerate its fallback):**
- [ ] **Vendor camera `setprop` toggles** — write `persist.vendor.camera.<key>=<val>` for known-good OnePlus / OPPO toggles to unlock hidden HFR / RAW modes that the standard `CameraManager` does not advertise. **Fallback (no root):** every code path that consumes a vendor key first calls `VendorKeyGuard.useIfAvailable { ... }`; if the key is absent the standard Camera2 path runs and the user sees a "Standard mode" badge in the HUD. Pure-data scaffolding shipped (`RootCapability.Feature.VendorSetProp` + `RootGate.evaluate(state)` returns `Disabled with fallback: standard Camera2 modes only` when state != Granted).
- [ ] **Camera HAL recovery** — `setprop ctl.restart cameraserver` to recover from HAL hangs without a full reboot. **Fallback (no root):** show a toast "Camera HAL hung — please power-cycle the device" and force-close the app. Pure-data scaffolding shipped (`RootCapability.Feature.CameraServerRestart`).
- [ ] **CPU governor pin during HFR** — write `performance` to `/sys/devices/system/cpu/cpu*/cpufreq/scaling_governor` to keep the SoC pinned to peak clocks during long high-frame-rate captures (avoids governor-induced frame drops on cold-start sequences). **Fallback (no root):** rely on Android's standard PerformanceHint API (`PowerManager.boostMode`) and accept that the kernel will downclock under thermal pressure; the HUD already surfaces the `phase9_thermal_*` warnings so the user can see when frames are being dropped. Pure-data scaffolding shipped (`RootCapability.Feature.CpuGovernorPin`).
- [ ] **Thermal trip override read** — read `/sys/class/thermal/thermal_zone*/trip_point_*_temp` to surface earlier user-facing thermal warnings (the standard `PowerManager.currentThermalStatus` API rounds the level to one of 5 buckets; the kernel exposes the actual °C). **Fallback (no root):** use `PowerManager.currentThermalStatus` + the existing `phase9_thermal_*.json` ringbuffer. Pure-data scaffolding shipped (`RootCapability.Feature.ThermalTripRead`).
- [ ] **System-wide logcat ringbuffer pull** — `logcat -b crash,events,system,radio` via `Runtime.exec("su -c logcat ...")` to capture HAL crashes for the diagnostic dump (the standard non-root `logcat` only sees the app's own UID + the main buffer, which filters out HAL-side crashes). **Fallback (no root):** the existing `DiagnosticsMode.dump()` writes a UID-filtered `logcat -d` plus the app's `*.dat` files — sufficient for app-side bugs but blind to HAL bugs. Pure-data scaffolding shipped (`RootCapability.Feature.LogcatSystemWide`).
- [ ] **Selective vendor-key probing** — read OEM vendor request keys that require `system_app` or root signature (e.g. `com.oplus.*` private namespace keys). **Fallback (no root):** the existing `DODGE_PROFILE.md` "vendor-key gaps" table tracks which OEM keys we cannot read and what we substitute (e.g. for OnePlus 13 OIS modes we will fall back to inferring stabilization from `LENS_INFO_AVAILABLE_OPTICAL_STABILIZATION` even when it returns `[OFF]`, with a lower-confidence "Software stabilization (assumed)" badge in the HUD). Pure-data scaffolding shipped (`RootCapability.Feature.VendorKeyProbe`).
- [ ] **Force-resolution overrides** — write `persist.vendor.camera.preview.size=<W>x<H>` for unusual configurations (e.g. capturing 8160×4592 video instead of the system-advertised 7680×4320). **Fallback (no root):** the standard `StreamConfigurationMap` filtering picks the closest supported size; the HUD shows the actual resolution so the user is never surprised by a silent downscale. Pure-data scaffolding shipped (`RootCapability.Feature.ResolutionOverride`).
- [ ] **Backlight brightness read for accurate exposure preview** — read `/sys/class/leds/lcd-backlight/brightness` to know the actual backlight cd/m² rather than the abstract Android brightness slider value. **Fallback (no root):** trust `WindowManager.LayoutParams.screenBrightness` and the user's HDR toggle. Pure-data scaffolding shipped (`RootCapability.Feature.BacklightRead`).

**V&V gate.**
- [ ] [ADB] On a non-rooted device the Root Only drawer renders with every feature disabled + the canonical "Requires root. Fallback: <X>" reason; the "Grant Su" button is enabled (so the user can tap it and see "No SU manager") but pressing it does NOT crash; every non-root fallback path produces a working capture.
- [ ] [ADB] On a rooted device (LineageOS 23 user-debug) tapping "Grant Su" surfaces a Magisk / KernelSU consent dialog; granting flips every Root Only feature to enabled; revoking flips them all back without an app restart.
- [ ] **JUnit-tested**: `RootCapability` + `RootGate` cover every state transition (`Unknown → NotAvailable`, `Unknown → AvailableNotGranted → Granted`, `Granted → Denied` after revoke); every `Feature` returns a non-blank `disabledReason` AND a non-blank `fallback` description. `RootCapabilityProbeTest` covers the static probe pure-data layer (path table, file-existence collation, `probeStatic` returns `NotAvailable` when the synthetic `FakeFs` has no SU paths and `AvailableNotGranted` when at least one path exists).
