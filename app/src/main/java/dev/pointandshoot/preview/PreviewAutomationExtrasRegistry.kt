 ...
 * |--------------------|-------------------|--------------------------------------------------------|
 * | [EXTRA_PNS_PREVIEW_AUTOMATION_IN_APP_VIDEO_SEC] | `pns_preview_automation_in_app_video_sec` | `inAppVideoAutomationSec`; clears RAW count, forces Auto dial + video-primary |
 * | [EXTRA_PNS_PREVIEW_VIDEO_RAW_SEC] | `pns_preview_video_raw_sec` | `rawVideoAutomationSec`; same video-primary sanitization as in-app video |
 ...
 * | [EXTRA_PNS_PREVIEW_DIAL] | `pns_preview_dial` | Set to `"Auto"` when video automation extras are present |
 * | [EXTRA_PNS_PREVIEW_PRIMARY_PHOTO] | `pns_preview_primary_photo` | Forced `false` when video automation; otherwise optional seed → `primaryPhoto` |
