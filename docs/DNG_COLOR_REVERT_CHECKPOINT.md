...
**Meaning:** All back cameras produce **openable** DNGs (pipeline pairing + RAW_SENSOR routing restored). Ultrawide and tele may still show **wrong color** in desktop RAW converters on legacy SKU-class devices.
...
**FM/ASN experiment (reverted May 2026):** Post-`DngCreator` **ForwardMatrix** / **AsShotNeutral** patching in **`Dng12Saver`** was tried and **reverted** — it broke wide and tele DNGs when applied to the wrong physical ids. **Do not reapply** without maintainer sign-off and USB proof. Color investigation uses **ReferenceCam** reference DNGs on-device; see **`docs/DNG_REFERENCE_APPS.md`**.
