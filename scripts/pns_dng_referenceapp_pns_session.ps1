...
.EXAMPLE
  .\scripts\pns_dng_referenceapp_pns_session.ps1 -Serial <serial>
  .\scripts\pns_dng_referenceapp_pns_session.ps1 -SkipPnsCapture -ReferenceAppDir hfr-runs\referenceapp_reference_*
  .\scripts\pns_dng_referenceapp_pns_session.ps1 -PnsStillModes standard,zsl,hdr -PullAltReferenceAppReference
#>
param(
    [string]$Serial = "",
    [int]$WaitSec = 55,
...
$sessionDir = Join-Path $projRoot "hfr-runs\dng_referenceapp_pns_session_$ts"
...
if (-not $SkipReferenceAppPull) {
    $pullArgs = @(
        "-File", (Join-Path $PSScriptRoot "pns_referenceapp_dng_reference_pull.ps1"),
        "-DngCount", "3",
        "-PnsCompareDir", "__skip__"
    )
    if ($Serial) { $pullArgs += @("-Serial", $Serial) }
    & powershell -NoProfile -ExecutionPolicy Bypass @pullArgs
    if ($LASTEXITCODE -ne 0) { throw "pns_referenceapp_dng_reference_pull failed" }
    $referenceappLatest = Get-ChildItem (Join-Path $projRoot "hfr-runs") -Directory -Filter "referenceapp_reference_*" |
        Sort-Object Name | Select-Object -Last 1
    if (-not $referenceappLatest) { throw "No referenceapp_reference_* folder after pull" }
    $ReferenceAppDir = $referenceappLatest.FullName
    Write-Host "[session] ReferenceCam -> $ReferenceAppDir"
} elseif ([string]::IsNullOrWhiteSpace($ReferenceAppDir)) {
    $referenceappLatest = Get-ChildItem (Join-Path $projRoot "hfr-runs") -Directory -Filter "referenceapp_reference_*" |
        Sort-Object Name | Select-Object -Last 1
    if ($referenceappLatest) { $ReferenceAppDir = $referenceappLatest.FullName }
}

if ([string]::IsNullOrWhiteSpace($ReferenceAppDir) -or -not (Test-Path $ReferenceAppDir)) {
    throw "ReferenceAppDir missing; run ReferenceCam captures first or omit -SkipReferenceAppPull"
}
...
function Try-PullAltReferenceAppDngs([string]$DestDir, [int]$Count = 3) {
    ...
    $pkgLine = (AdbShell "pm path $AltReferenceAppPackage 2>/dev/null").Trim()
    if (-not $pkgLine) {
    ...
    if ($picked.Count -eq 0) {
        return @{ ok = $false; reason = "no_altreferenceapp_dng_in_dcim"; dir = $DestDir }
    }
    $i = 0
    foreach ($item in $picked) {
        $i++
        $local = Join-Path $DestDir ("altreferenceapp_{0:D2}.dng" -f $i)
        if ($Serial) {
...
$motionCamDir = ""
$motionCamPull = $null
if ($PullAltReferenceAppReference) {
    $motionCamDir = Join-Path $sessionDir "altreferenceapp_reference"
    $motionCamPull = Try-PullAltReferenceAppDngs $motionCamDir 3
    Write-Host "[session] AltReferenceApp pull: $($motionCamPull.reason) (count=$($motionCamPull.count))"
}
...
if ($comparePnsDir -and (Test-Path (Join-Path $comparePnsDir "M23_wide.dng"))) {
    $compareArgs = @(
        ...
        "-ReferenceAppDir", $ReferenceAppDir,
...
$lines.Add("|----------|------|")
$lines.Add("| ReferenceCam pull | ``$ReferenceAppDir`` |")
...
if ((Test-Path $pyTag)) {
    $psFiles = @(
        (Join-Path $ReferenceAppDir "referenceapp_01.dng"),
        (Join-Path $ReferenceAppDir "referenceapp_02.dng"),
        (Join-Path $ReferenceAppDir "referenceapp_03.dng")
    ...
    if ($motionCamPull -and $motionCamPull.ok -and $motionCamDir) {
        ...
        if ($mcFiles.Count -gt 0) {
            $lines.Add("")
            $lines.Add("## AltReferenceApp tags (newest DCIM pull)")
            $lines.Add("``````")
