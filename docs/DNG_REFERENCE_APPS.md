...
## ReferenceCam (`com.riseupgames.referenceapp2`) — primary reference
...
|--------|-------------------|
| Pull latest non–Point & Shoot DNGs + tag diff vs P&S | `scripts/pns_referenceapp_dng_reference_pull.ps1` → `hfr-runs/referenceapp_reference_*` |
| logcat + `dumpsys media.camera` during manual captures | `scripts/pns_referenceapp_adb_forensics.ps1` |
| APK string grep (`DngCreator`, `RAW_SENSOR`, `setPhysicalCameraId`, …) | Pulled into `referenceapp_reference_*/apk/` by reference pull script |
...
## ExternalCameraApp — deprioritized on this device

[ExternalCameraApp](https://sourceforge.net/p/externalcameraapp/code/) (GPLv3) is useful for **reading** a minimal Camera2 DNG path:
...
**On legacy SKU, ExternalCameraApp produces the same broken aux DNG color as Point & Shoot** — not a product color reference for this HAL. Keep source links for API patterns only; do not treat ExternalCameraApp parity as a color fix.
