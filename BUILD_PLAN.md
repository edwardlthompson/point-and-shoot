...
- [ ] Highlight-weighted metering (protect 95th percentile luma; Ricoh GR style) — `H` mode wired in `CommandDialMode.H`; algorithm scaffolded in `HighlightMeter.suggestEvCorrection()` (pure function over a 256-bin luma histogram, EV-clamped, **JUnit-tested** in `HighlightMeterTest.kt`); preview-frame histogram pipeline pending Phase 1 capture engine.
...
### Part 4 requirements
- [ ] Typography: JetBrains Mono for all technical readouts — `PnsTypography` is fully monospaced (`MonoFamily = FontFamily.Monospace`); JetBrains Mono TTF (SIL OFL 1.1) still needs to be vendored as a font asset and bound to `MonoFamily`.
...
- [ ] **X-Rite ColorChecker Passport (24-patch subset)** — same coords/values strategy; same non-redistribution stance on the trademark/image. Pending: the Passport reference values overlap with Classic 24 for the color patches; the additional Creative Enhancement / portrait-warming patches need to be transcribed from the X-Rite published data sheet first.
...
- [ ] `Dng12Saver` extension writes `ColorMatrix1` / `ForwardMatrix1` derived from the active `CalibrationProfile` so desktop tools (`darktable`, `RawTherapee`) inherit the calibration on the RAW path **without** baking it in.
...
- [ ] **ACES sRGB → ACEScct** (Apache-2.0, AMPAS OpenColorIO config) — neutral working-space conversion. Pending the Gradle `downloadBundledLuts` task; will land as an asset-backed entry in `LutCatalog`.
- [ ] **ACEScct → sRGB Display** (Apache-2.0, AMPAS OpenColorIO config) — display transform. Pending `downloadBundledLuts`.
- [ ] **Filmic (Blender)** sRGB → log (Apache-2.0, from Blender's `filmic-blender` repo) — gentle highlight rolloff for HDR-to-SDR delivery. Pending `downloadBundledLuts`.
...
- [ ] **`.3dl`** (Autodesk; publicly documented) — secondary import-only.
...
#### Apply path
- [ ] **Live preview / video**: GLES 3.0 fragment shader sampling a `sampler3D` RGB texture with hardware trilinear interpolation. Shader template in `app/src/main/assets/shaders/lut_apply.frag.glsl`. Identity LUT bypasses the shader stage entirely.
...
- [ ] `LutShader.kt` — GLES setup: 3D-texture upload from `Lut3D`, fragment-shader binding, identity bypass when `Lut3D.isIdentity()`. Pending the GLES preview / video pipeline.
- [x] Tests shipped: `LutPipelineTest` (12), `LutCatalogTest` (8), `Lut3DTest` (8), `BuiltInLutsTest` (11), `CalibrationMathTest` (12), `CalibrationToLutTest` (8) — 59 new JUnit tests, all green via `:app:testDebugUnitTest` (full suite: 19 suites / 147 tests / 0 failures / 0 errors / 0 skipped). Reference parity vs FFmpeg `lut3d` is intentionally NOT a unit-test gate (FFmpeg is not on every developer's PATH); we instead test that our CPU apply path matches a profile's direct `apply()` to within 1 LSB on 8-bit (which is the property a GLES↔CPU reconciliation needs anyway).
...
- [ ] Add a README-renderable icon asset:
  ...
  - [ ] Embed it at the top of `README.md` (placeholder comment already present).
...
- [x] Refresh `README.md` to be an enticing pitch and onboarding doc:
  ...
  - [ ] Screenshots section (placeholder until UI exists) — icon comment placeholder added; awaiting UI screenshots.
