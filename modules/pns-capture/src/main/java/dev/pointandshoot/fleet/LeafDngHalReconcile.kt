...
object LeafDngHalReconcile {
    ...
    fun shouldReconcileLeafDngMetadata(sessionCameraId: String): Boolean =
        shouldReconcileLeafDngMetadataWhen(
            deviceApplies = LeafDngFleetPolicies.active.appliesToDevice(),
            backend = LeafDngFleetPolicies.active.stillDngBackend(),
     ...
     */
    fun shouldReconcileLeafDngMetadataWhen(
        ...
        wideLeafCalibrationForAuxDng: Boolean = LeafDngFleetPolicies.active.useWideLeafCalibrationForAuxDng(),
        ...
        if (sessionCameraId == LeafDngFleetPolicies.active.canonicalWide) {
            return false
        }
        if (
            LeafDngFleetPolicies.active.useLegacyAsnReconcileOnly() &&
            (sessionCameraId == LeafDngFleetPolicies.active.canonicalUw ||
                sessionCameraId == LeafDngFleetPolicies.active.canonicalTele)
        ) {
            return true
    ...
    private fun leafRearIds(): Set<String> =
        setOf(
            LeafDngFleetPolicies.active.canonicalUw,
            LeafDngFleetPolicies.active.canonicalWide,
    ...
    fun applyPostDngCreatorPatches(
        ...
        if (useAsnOnlyPath(sessionCameraId)) {
            val bayerAsn =
                if (sessionCameraId == LeafDngFleetPolicies.active.canonicalTele) {
                    preWriteBayerAsn
    ...
    private fun useReferenceAppReferenceCalibrationPath(sessionCameraId: String?): Boolean {
        if (!LeafDngFleetPolicies.active.useReferenceAppReferenceCalibration()) return false
        if (sessionCameraId == null) return false
    ...
    private fun useAsnOnlyPath(sessionCameraId: String?): Boolean {
        ...
        if (LeafDngFleetPolicies.active.useReferenceAppPureDngSave()) return false
        if (DngSaveBisectState.forceLegacyAsnReconcile) return false
        if (sessionCameraId == null) return false
        if (!LeafDngFleetPolicies.active.useLegacyAsnReconcileOnly()) return false
        return sessionCameraId == LeafDngFleetPolicies.active.canonicalUw ||
            sessionCameraId == LeafDngFleetPolicies.active.canonicalTele
    }
    ...
    private fun useWideLeafCalibrationPath(
        ...
        if (!LeafDngFleetPolicies.active.useWideLeafCalibrationForAuxDng()) return false
        return sessionCameraId == LeafDngFleetPolicies.active.canonicalUw ||
            sessionCameraId == LeafDngFleetPolicies.active.canonicalTele
    }
     ...
     *
     * **Loadability:** gated by [LeafDngFleetPolicies.active.useWideLeafCalibrationForAuxDng] + maintainer USB proof
            ...
            correctedStillGains != null -> {
                ...
                Log.i(
                    ...
                    "wide-cal AsShotNeutral from Op13 still-request gains auxCam=$sessionCameraId " +
                        "profileCam=${LeafDngFleetPolicies.active.canonicalWide}",
                )
            }
            else -> {
    ...
    private fun useHalColorCalibrationPath(sessionCameraId: String?): Boolean {
        ...
        if (DngSaveBisectState.forceLegacyAsnReconcile) return false
        if (sessionCameraId == null) return false
        if (!LeafDngFleetPolicies.active.useHalColorCalibrationReconcile()) return false
        return sessionCameraId == LeafDngFleetPolicies.active.canonicalUw ||
            sessionCameraId == LeafDngFleetPolicies.active.canonicalTele
    }
    ...
    private fun applyOp13AsnOnlyReconcile(
        ...
        var bytes = original
        if (sessionCameraId == LeafDngFleetPolicies.active.canonicalTele && preWriteBayerAsn != null) {
            bytes =
        ...
        if (correctedStillGains != null) {
            ...
            if (gains != null) {
                val forPatch =
                    if (DngSaveBisectState.skipHalWbGainCorrection) {
                        gains
                    } else if (sessionCameraId == LeafDngFleetPolicies.active.canonicalTele) {
        ...
        if (LeafDngFleetPolicies.active.useLegacyLeafAuxColorReconcile()) {
            val model = Build.MODEL?.lowercase().orEmpty()
            DngForwardMatrixFix.get(model, sessionCameraId)?.let { fmOverride ->
                bytes = TiffDngColorMatrixPatch.patchForwardMatrix(bytes, fmOverride)
     ...
     * [LeafDngFleetPolicies.active.useHalColorCalibrationReconcile] is on (explicit bisect / fleet policy).
     */
    ...
    private fun preferBayerAsnForSession(sessionCameraId: String?): Boolean {
        ...
        return sessionCameraId == LeafDngFleetPolicies.active.canonicalTele
    }
