...
**JPEG Artist / Copyright:** `PnsJpegCreditPrefs` (`pns_jpeg_credits`). Optional fields applied only in `StillCaptureMetadata.applyToJpegUri`. **Never** on DNG (`applyToDngUri` stays in-place TIFF only — no `ExifInterface`). Privacy strip skips these tags. About → JPEG artist / copyright.

**Gallery groups:** `GalleryCaptureGroups` treats same-timestamp RAW+JPEG and `bkt-` / `hdr-` / `nightscape` suffixes as one capture for share/delete. **Power profile** (`pns_power_profile`) caps preview FPS (Balanced 60, Endurance 30) before adaptive thermal/battery. **Widget** `pointandshoot://preview?shoot=1` sets `pns_preview_composed_still`.

**Still / video storage floors:** `PreviewStillStorageGate` — 48 MB per planned still frame (HDR/burst/BKT/NightScape scale). `remainingShots` is available/floor. Intervalometer needs 2× before arming, then the planned frame count on each photo tick. Video-mode intervalometer uses encode remaining (`shouldRefuseRecordStart` / stop-at-0), not one still floor. Hold-burst start and each shot budget 2× when RAW+JPEG both write; self-timer fire re-checks planned frames. NightScape / BKT re-check remaining frames before each shot. Still-queue enqueue and drain abort when the planned floor is missing. Few-stills toast at 3× the floor. `PreviewVideoStorageEstimate.shouldRefuseRecordStart` refuses a new record when remaining time is under **1** minute; a toast warns under **5** even if the overlay is hidden; `shouldStopRecordForEmpty` stops a running clip at **0**. `PreviewFreeSpace` is the shared DCIM/cache StatFs helper — fail open if missing. APK Install needs GitHub `size` + 8 MB slack (or a 56 MB budget) on `cacheDir`, refuses over **150 MB**, and pins dest under `cache/updates`.
...
| Intervalometer | `intervalometerIntervalSec` + `intervalometerRunning` | Timed stills while preview is open (photo mode, not recording). Optional ISO ramp via `PnsProductPrefs.ramp_*`. |
| Geotag mode | `PnsProductPrefs.geotag_mode` Off/Coarse/Precise | Coarse rounds lat/lon to 0.01°. `CaptureLocationBridge.snapshot` applies the mode. Never writes EXIF via `ExifInterface` on DNG. |
| Wear remote | BLE GATT `8f7a0001-…` + LAN `POST /remote` | Watch APK `:wear`. Modes: Remote (shutter/video/chapter) or Timer (watch 3/5/10s or phone 5s). `timer_cancel` / BLE `0x11` cancels the phone delay. BLE rescan on disconnect. |
| HDMI / MJPEG | `PnsExternalOutput` + port **28770** | Presentation prefers `TYPE_EXTERNAL` HDMI, 450 ms settle + 6 retries. PixelCopy from `SurfaceView`/`TextureView`; last-good JPEG kept on miss. MJPEG `/mjpeg` + `/snapshot.jpg`. |
| USB webcam | Tray Photo → Video → **Webcam** · `PnsUsbWebcam` | **Camera / Zoom / Teams:** Lineage/AOSP USB **Webcam** (UVC gadget + `DeviceAsWebcam`). Windows loads inbox **usbvideo.sys** (`Android Webcam` / USB Video Device). Automate with `scripts/pns_usb_webcam_windows.ps1` (`svc usb setFunctions uvc`, Settings tap fallback). P&S must stay stopped so DeviceAsWebcam owns the camera. In-app USB button opens Connected devices; if Root **Grant Su** is already stored it runs the same `svc` commands. **HTTP fallback (OBS only, not Zoom/Teams):** `PnsWebcamEncoder` lean video session, `GET /h264` + `/mjpeg`. ADB `--ez pns_preview_webcam true`. |
| Library desk | Gallery overflow | Compare, cull, keywords, travel, trim, bake, vault, Immich/WebDAV, `/proofing`, redact, SHA-256 receipt. |
