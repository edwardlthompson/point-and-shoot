...
.EXAMPLE
  .\scripts\pns_referenceapp_live_forensics.ps1 -Serial <serial>
  .\scripts\pns_referenceapp_live_forensics.ps1 -TryUiAutomation -PerLensSec 12
  .\scripts\pns_referenceapp_live_forensics.ps1 -ManualOnly -PerLensSec 25
#>
param(
    [string]$Serial = "",
    [int]$PerLensSec = 20,
...
$outDir = Join-Path $projRoot "hfr-runs\referenceapp_live_forensics_$ts"
...
# Calibrated from hfr-runs/referenceapp_lens_row.png May 2026.
...
function Get-LatestReferenceAppDngPaths {
...
function Get-CameraServiceEventsFromLogcat([string]$logFile, [string]$sinceMarker) {
    ...
    foreach ($line in $lines) {
        ...
        if ($line -match "CameraService:.*connect call \(PID.*referenceapp2.*camera ID (\d+)") {
            [void]$out.Add($line.Trim())
        }
        if ($line -match "CameraService:.*(CONNECT|DISCONNECT) device (\d+).*referenceapp2") {
            [void]$out.Add($line.Trim())
        }
    }
...
function Wait-NewDng([int]$sinceEpoch, [int]$timeoutSec) {
    ...
    while ([DateTimeOffset]::UtcNow.ToUnixTimeSeconds() -lt $deadline) {
        $new = Get-LatestReferenceAppDngPaths | Where-Object { $_.Mtime -ge $sinceEpoch - 2 }
...
# --- logcat background ---
$logPath = Join-Path $outDir "referenceapp_live_logcat.txt"
$logTags =
...
Write-Host "Logcat PID $($logProc.Id) -> referenceapp_live_logcat.txt"
Write-Host ""
...
$preDng = Get-LatestReferenceAppDngPaths
$logMarker = "referenceapp_live_forensics_start"

Invoke-Adb @("shell", "am", "start", "-n", "$ReferenceAppPackage/.activities.MainActivity") | Out-Host
...
foreach ($lens in $script:LensTaps) {
    ...
    if ($gotDng) {
        $local = Join-Path $outDir "referenceapp_$($lens.label)_$($lens.camId).dng"
...
$filterPath = Join-Path $outDir "referenceapp_live_logcat_filtered.txt"
$patterns = @(
...
$py = Join-Path $PSScriptRoot "referenceapp_live_parse.py"
if ((Test-Path $py) -and (Get-ChildItem $outDir -Filter "referenceapp_*.dng").Count -gt 0) {
    Write-Host ""
    Write-Host "[parse] referenceapp_live_parse.py (optional, 60s cap)..." -ForegroundColor Cyan
    ...
    if ($done) {
        ...
        Write-Host "WARN: referenceapp_live_parse.py timed out (skipped)" -ForegroundColor Yellow
    }
...
Write-Host "  referenceapp_live_logcat.txt (full)"
Write-Host "  referenceapp_live_logcat_filtered.txt"
