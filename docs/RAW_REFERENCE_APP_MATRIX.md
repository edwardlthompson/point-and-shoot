# RAW reference app matrix (P&S vs ReferenceCam vs MotionCam)

Triangulation for **Milestone 13** fleet RAW work on **legacy device** (`legacy SKU`). Refresh after APK updates via `pns_proshot_apk_decompile.ps1` and `pns_motioncam_apk_decompile.ps1`.

| Capability | Point & Shoot (today) | ReferenceCam (`com.riseupgames.proshot2`) | MotionCam (`com.motioncam.pro`) |
|------------|----------------------|----------------------------------------|-------------------------------------|
...
| **Aux DNG color (legacy SKU)** | **MotionCam-inspired** still path (May 2026): `RAW_SENSOR@activeArray`, no leaf ASN reconcile | ReferenceCam path **retired** on legacy device after color failure | Native DNG SDK; triangulate behavior not copy JNI |
...
|--------|-----------------|-----------|
| **13.2** Fleet profile | ReferenceCam `C0527f` + MotionCam `NativeCameraRawOutput` / `NativeCameraInfo` | `DODGE_PROFILE.md`, probe hub |
| **13.3** Still DNG — **Standard** default (legacy device) | **MotionCam-inspired** (`RAW_SENSOR@activeArray`, map-first shading, no leaf ASN reconcile) | ReferenceCam for other devices / bisect |
| **13.8** ZSL + HDR still modes | MotionCam `CaptureZslImage` / `CaptureHdrImage` **behavior** | Same ReferenceCam DNG save as 13.3 |
...
| ReferenceCam | `hfr-runs/proshot_apk_decompile/` | `docs/PROSHOT_APK_FLEET_ANALYSIS.md` |
