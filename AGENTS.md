...
| `pns_usb_webcam_windows.ps1` | Enable Lineage/AOSP **USB Webcam** (`svc usb setFunctions uvc` + Settings tap fallback). Windows Camera / Zoom / Teams see **Android Webcam** via inbox **usbvideo.sys**. Force-stops P&S so DeviceAsWebcam can own the camera. `-LaunchCamera` opens the Windows Camera app. |
...
| `pns_release_packaging.ps1` | Sprint **14.13**: **`assembleRelease`**, rename **`Point-and-Shoot-{versionName}.apk`**, **`zipalign -c -v 4`**, **apksigner** (no `gh`). **Fails closed** without production signing unless **`-AllowDebugKey`**. Naming: **`pns_release_naming.ps1`**. |
| `pns_github_release.ps1` | **GitHub release orchestrator** — `-PrepareOnly` (stable semver + CHANGELOG + coverage + About tag + wear version), `-Publish` (**signed** APK, title **`Point & Shoot {version}`**, changelog body + APK + sha256). Skill: `.cursor/skills/github-release/SKILL.md`. |
| `pns_release_signing_bootstrap.ps1` | Create gitignored `release.keystore` + `keystore.properties`; optional `-SetGitHubSecrets` + `-UpdateExpectedJson`. Idempotent unless `-ForceCreate`. |
