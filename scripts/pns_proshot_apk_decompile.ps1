<#
.SYNOPSIS
  Pull ProShot base.apk, decompile with jadx, run referenceapp_decompile_scan.py.
...
if (Test-Path $scanPy) {
    python $scanPy $src (Join-Path $outRoot "scan.json") --profile all
}

Write-Host "[proshot_decompile] done. APK: $apk"
Write-Host "[proshot_decompile] analysis: docs/PROSHOT_APK_FLEET_ANALYSIS.md"
