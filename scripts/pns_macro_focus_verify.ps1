...
.SYNOPSIS
    ...
    Launches app in Macro (CommandDialMode.Macro) via ADB extra, verifies:
      1. App switches to UW camera (macroMode autoSwitchUW log)
...
Invoke-AdbCmd shell am start -n "dev.pointandshoot/.MainActivity" `
    ...
    --es pns_preview_dial MACRO 2>&1 | Out-Null
...
# --- Gate checks ---
$uwSwitch      = $logLines -match "macroMode autoSwitchUW"
...
Write-Host "  UW auto-switch      : $uwSwitch"
...
# Overall pass: UW switch + no cam errors mandatory; macro vendor key is device-dependent (OPLUS only)
$overallPass = $uwSwitch -and -not $camErrors

$result = [ordered]@{
    timestamp      = $stamp
    passed         = $overallPass
    uwAutoSwitch   = $uwSwitch
...
if ($overallPass) {
    ...
    if (-not $macroApplied) {
        Write-Host "  NOTE: vendor macro key not applied (non-OPLUS device or key not advertised) — UW switch + no errors sufficient" -ForegroundColor Yellow
    }
    if (-not $afLocked) {
    ...
    if (-not $uwSwitch) { Write-Host "  FAIL: UW camera auto-switch not detected in logcat" -ForegroundColor Red }
