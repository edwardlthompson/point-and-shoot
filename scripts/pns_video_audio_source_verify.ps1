...
<#
.SYNOPSIS
  Sprint **15.24** — video audio source logged during in-app recording.

.EXAMPLE
  .\scripts\pns_video_audio_source_verify.ps1 -Serial b5214fc6 -SkipAssemble
#>
param(
    [string]$Serial = "",
    [switch]$SkipAssemble,
...
$outDir = Join-Path $repo "hfr-runs\video_audio_source_verify_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
...
Write-Host "[audio_source] waiting for in-app video record..."
Start-Sleep -Seconds $ScreencapDelaySec

$remain = [Math]::Max(0, $rec + 12 - $ScreencapDelaySec)
if ($remain -gt 0) {
    Write-Host "[audio_source] waiting ${remain}s for clip finalize..."
    Start-Sleep -Seconds $remain
}

$logPath = Join-Path $outDir "logcat.txt"
adb -s $Serial logcat -d -v threadtime -s PNS.MCVideoRec:I PNS.VideoRec:I PNS.AdbValidation:I AndroidRuntime:E 2>&1 |
    Out-File -Encoding utf8 $logPath
$log = Get-Content $logPath -Raw

$audioSource = $log -match "audioSource=CAMCORDER"
$videoSaved = $log -match "inAppVideoSaved ok=true"
$pass = $audioSource -and $videoSaved

$gate = [ordered]@{
    pass = $pass
    audioSourceCamcorder = [bool]$audioSource
...
if (-not $pass) {
    Write-Host "FAIL video audio source gate — $outDir" -ForegroundColor Red
    exit 1
}
Write-Host "PASS video audio source gate — $outDir" -ForegroundColor Green
