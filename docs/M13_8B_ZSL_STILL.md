...
- **Ring:** `ZslStillFrameRing` (capacity **6** on legacy device via `LegacyDeviceFleetPolicy.zslStillRingCapacity()`).
...
## USB verification (legacy SKU)

```powershell
.\scripts\pns_still_mode_benchmark.ps1 -Serial legacy serial -Mode zsl -Repeats 1
```
