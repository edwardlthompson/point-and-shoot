...
**Package:** `com.riseupgames.referenceapp2`  
**APK on disk:** `hfr-runs/referenceapp_apk_decompile/referenceapp2_base.apk` (pulled from device May 2026)  
**Decompile:** `hfr-runs/referenceapp_apk_decompile/jadx_sources/` (jadx 1.5.1)  
**Refresh:** `.\scripts\pns_referenceapp_apk_decompile.ps1`  
**Needle scan:** `hfr-runs/referenceapp_apk_decompile/scan.json` via `scripts/referenceapp_decompile_scan.py`
...
6. **Validation** — `pns_dng_referenceapp_pns_session.ps1` in **daylight** for 3/3 UW; dark room is stress-only for HAL failures.
...
- [`DNG_REFERENCEAPP_ADB_FINDINGS.md`](DNG_REFERENCEAPP_ADB_FINDINGS.md) — dumpsys CONNECT 3/2/4.
