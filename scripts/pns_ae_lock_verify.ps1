...
<#
.SYNOPSIS
  Sprint **15.26** — preview AE lock via CONTROL_AE_LOCK + readout padlock.

.EXAMPLE
  .\scripts\pns_ae_lock_verify.ps1 -Serial b5214fc6 -SkipAssemble
#>
param(
    [string]$Serial = "",
    [switch]$SkipAssemble,
...
$outDir = Join-Path $repo "hfr-runs\ae_lock_verify_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
...
adb -s $Serial shell am start -W -n "$pkg/.MainActivity" `
    ...
    --ez pns_preview_ae_lock true 2>&1 | Out-Null

Write-Host "[ae_lock] waiting ${WaitSec}s for preview + AE lock..."
Start-Sleep -Seconds $WaitSec

$capPath = Join-Path $outDir "ae_lock_preview.png"
...
$aeLock = $log -match "aeLock=true"
$seed = $log -match "preview seeded aeLock=true"
$screencapOk = Test-Path $capPath
$pass = $aeLock -and $seed -and $screencapOk

$gate = [ordered]@{
    pass = $pass
    aeLockLogged = [bool]$aeLock
    aeLockSeeded = [bool]$seed
    screencap = $capPath
    outDir = $outDir
}
...
if (-not $pass) {
    Write-Host "FAIL AE lock gate — $outDir" -ForegroundColor Red
    exit 1
}
Write-Host "PASS AE lock gate — $outDir" -ForegroundColor Green
